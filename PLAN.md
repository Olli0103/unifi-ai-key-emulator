...
The earlier one-use event permits were consumed. Automatic inventory refresh, 12-per-hour continuous admission and private terminal-job rollover are implemented with synthetic tests but are not deployed or native-verified. Protect-side all-camera dispatch, fair scheduling and multi-day endurance remain open. A separate AI Port candidate recovered an existing adoption in Protect 7.3.60 and stayed Online after restart. A bounded legacy G3 trial paired the camera, decoded its stream and completed three local RF-DETR Nano model calls without error. Those frames produced no object observations. A later expiring capability probe made Protect send native smart-detection settings; enabling **Record AI Events** changed its requested type count from zero to one. A new expiring, single-camera person-policy acknowledgement and one-event publisher have synthetic tests but no native verification. In a subsequent live trial, Protect requested person reverification despite its Person control appearing off; the candidate rejected all four settings requests, made no detector calls and emitted no smart event. The global Second Stage setting was restored to All, the camera was unpaired, and the AI Port returned to passive mode while the separate AI Key remained Online. The optional local observation path, bounded frame queue, in-memory temporal tracker and fail-closed full-frame settings parser have synthetic coverage. A CPU-only Apple container passed a real-checkpoint smoke test on a public image. None of these checks proves native smart-event delivery or model accuracy on camera footage. Fresh AI Port adoption, durable multi-camera streams, native smart detections and alarms remain `needs_evidence`. Native search, structured recognition, speech and NAS operation remain unverified or unimplemented. The existing E5 query adapter and database preparation are not proof of native retrieval. Static research on Protect 7.2.105 and AI Key 2.2.8 is version-specific evidence, not proof of every 7.3.60 contract.
