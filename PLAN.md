...
As of 23 September 2026, the AI Key Apple container has native adoption, control, credential rotation, disabled factory login and reconnect evidence. Protect 7.3.60 on a UDM Pro Max accepted one automatic caption each on G5 Flex and G4 Instant from a configured OpenAI model. Both appeared in the native event panel after a full page reload. The earlier adoption test used Protect 7.3.56. These one-use trials do not establish continuous operation. See [build results](BUILD-RESULT.md) and [description acceptance](docs/basic-descriptions.md).

The earlier one-use event permits were consumed. Automatic inventory refresh, 12-per-hour continuous admission and private terminal-job rollover are implemented with synthetic tests but are not deployed or native-verified. Protect-side all-camera dispatch, fair scheduling and multi-day endurance remain open. A separate AI Port candidate recovered an existing adoption in Protect 7.3.60 and stayed Online after restart. A bounded legacy G3 trial paired the camera and ran the local RF-DETR Nano model on its stream. An expiring capability probe made Protect send native smart-detection settings; enabling **Record AI Events** changed its requested type count from zero to one. Protect also requested person reverification despite its Person control appearing off. The candidate validates that policy and drops observations at or below its upper confidence bound instead of claiming to reverify them. Protect 7.3.60 acknowledged this one-camera policy; at least 100 subsequent G3 model calls produced no supported objects or native event. A G5 camera sent a configured smart zone that the candidate rejected; a separate three-frame local-only check on its stream produced three supported observations and one internal track. A new bounded primary-zone parser and conservative whole-box polygon gate have synthetic coverage but no live zone acceptance yet. Both test cameras were unpaired, AI Port ingress returned to passive mode, and the separate AI Key remained Online. The optional local observation path, bounded frame queue, in-memory temporal tracker and fail-closed settings parser have synthetic coverage. A CPU-only Apple container passed a real-checkpoint smoke test on a public image. None of these checks proves native smart-event delivery or model accuracy on camera footage. Native zone enforcement, fresh AI Port adoption, durable multi-camera streams, native smart detections and alarms remain `needs_evidence`. Native search, structured recognition, speech and NAS operation remain unverified or unimplemented. The existing E5 query adapter and database preparation are not proof of native retrieval. Static research on Protect 7.2.105 and AI Key 2.2.8 is version-specific evidence, not proof of every 7.3.60 contract.
...
Deployment must detect the current camera inventory and reconcile the number of AI Port instances automatically. Each instance has a distinct reachable LAN address and fixed HTTPS 443 listener; the AI Key retains HTTPS 8080. A host launcher creates the required port bindings before containers start, persists each identity, and refuses to start an unaddressed or port-conflicting instance. Camera count does not imply one new TCP port per camera. A native Protect-camera pairing used outbound RTSP TCP 7447 from the AI Port to the controller; ONVIF transport and any additional listeners remain `needs_evidence`. Inventory changes must not silently reassign adopted identities or paired cameras.
