...
During an enabled stream diagnostic, a successful stream-control response sends a per-camera `EventAIPortStatus` with the observed streaming state. Smart-detection and audio-event readiness are false by default. A separate expiring, one-camera probe temporarily sends `EventFeatureFlagsUpdated` with a person capability and reports smart readiness. The candidate classifies a narrow `ChangeSmartDetectSettings` subset for the exact camera: full-frame person, vehicle and animal detection with bounded event timing and no zones, lines, exclusions, tamper, access triggers, PTZ or enabled second-stage policy. The settings-only probe counts matching requests and still replies unsupported; it never retains the nested policy.

An additional private `diagnostic_event_until` field can enable one person-event trial. It must equal both the stream and smart-probe deadline, which can be at most ten minutes ahead. It also requires a local detector policy with 2-120 frame attempts. Only an exact, full-frame person setting for the selected camera, with person reverification disabled, receives an acknowledgement. A later disabled or unsupported setting revokes it. After two overlapping model observations, the in-memory tracker may send one `EventSmartDetect` enter for that person and a leave when the track disappears. The payload includes a wall-clock timestamp and normalized box, but no image, face, plate, speech or camera credential. This wire shape has synthetic tests only. In a bounded Protect 7.3.60 trial, the live G3 camera's policy still requested person reverification despite the visible Person control appearing off. The candidate rejected that policy, ran no detector call and emitted no smart event. Protect acceptance, original-camera timeline persistence and alarms remain `needs_evidence`. The default passive profile emits no smart events.
