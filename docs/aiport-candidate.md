...
During an enabled stream diagnostic, a successful stream-control response sends a per-camera `EventAIPortStatus` with the observed streaming state. It always reports smart-detection and audio-event readiness as false because the candidate has no validated native smart-event publisher. This event remains part of the expiring one-camera diagnostic, not a permanent AI Port implementation. The candidate can now classify a narrow `ChangeSmartDetectSettings` subset for the exact camera: full-frame person, vehicle and animal detection with bounded event timing and no zones, lines, exclusions, tamper, access triggers, PTZ or second-stage policy. It counts only matching requests and still replies unsupported. It neither retains the nested policy nor claims Protect 7.3.60 uses this exact shape. Settings enforcement and native event delivery remain `needs_evidence`.
