...
For a controlled hello experiment, a private config may include `diagnostic_hello_until`, a Unix timestamp no more than ten minutes ahead. This is off by default. During the window, the candidate sends a minimal `ubnt_avclient_hello`, answers only the controller's parameter-agreement request, records allowlisted command names without payloads, and closes the WebSocket at expiry. A reconnect after expiry is passive. The diagnostic does not answer `GetStreamList` or stream-control requests and must not be presented as camera pairing support.
