...
This profile is for bounded protocol tests. It presents a separate AI Port identity, keeps an outbound certificate-pinned camera WebSocket to Protect and exposes an HTTPS management listener. Camera ingress is off unless a private, expiring one-camera diagnostic policy enables it. The profile has no camera credentials or AI detection. Its optional adoption handshake has passed synthetic tests but has not been tried against Protect 7.3.60. One legacy camera paired during a bounded stream test; detection delivery remains unverified.
