...
This profile presents a separate AI Port identity, keeps an outbound certificate-pinned camera WebSocket to Protect and exposes an HTTPS management listener. Camera ingress is off by default. A private `paired_stream` policy can keep one exact camera paired and decoding. Explicit live or expiring diagnostic policies control inference and event publication. The profile has no camera credentials and does not advertise AI detection by default. Its management-token adoption handshake has passed synthetic tests; a separate existing-device reconnect was accepted by Protect 7.3.60 and survived a container restart. On 24 September 2026, the deployed local detector published live Person events from a paired G3 Instant. Protect showed two new results from the later three-camera run under that camera's Person filter after a full page reload, and each linked to the original camera's playback. A further owner walk after the two-frame-per-second update raised that camera's Person results from seven to nine; the newest result linked to a playable original-camera recording and the Smart strip showed two new Person thumbnails. The same adopted AI Port had three cameras paired and all three streams decoding. Event delivery from the other two cameras, measured recall and feature parity remain unverified.
...
The watch caps its runtime at ten minutes and discards event bodies. `--camera-name` must exactly identify one currently eligible camera; omit it to watch the full selected scope. It counts `add` and `update` messages for that validated camera set, including smart-video and person fields on either message. For targeted smart-video messages, `smart_type_states` separately counts an omitted `smartDetectTypes` key, explicit null, empty list, list containing person, or another nonempty list. The watcher also correlates camera and event IDs in bounded memory: `targeted_events_with_person_class` counts distinct events for which Protect explicitly announced Person, and `targeted_empty_to_person_transitions` counts distinct events whose explicit empty class list later changed to Person. It never prints or saves those IDs. An omitted or null field does not erase an earlier explicit state. With `--progress-jsonl`, the watcher flushes count-only snapshots when the subscription opens and after each targeted event or invalid message, followed by a `final` line if it finishes. If an operator interrupts a stuck watch, the last `progress` line remains partial evidence; it is not a completed watch. These are subscription observations, not a readback of the saved event. A matching subscription message proves Protect announced an event for that camera during the watch. It does not identify whether the AI Port or the camera's onboard model caused it, and it does not prove recording-timeline persistence. Check the original camera timeline after an event appears. In the third bounded recorded-frame trial below, the targeted smart-video add and update each carried an explicit empty `smartDetectTypes` list.
...
This opt-in mode accepts Protect's validated one-camera smart settings, including zone and reverification checks. It sends native enter, moving and leave events and offers an event snapshot through the pinned controller upload path. The rolling-hour event budget is now charged to a private state file before each enter, so a restart does not reset it. Corrupt state, clock rollback or an uncertain write denies new enters. A detector failure withdraws smart readiness without unpairing the camera. The paired stream remains available when the policy is removed. Multi-person tracking, simultaneous classes, sustained resource use, and loaded Protect timeline/filter behavior still need native validation. Do not treat this single-camera mode as AI Port feature parity.
...
During an enabled stream diagnostic, a successful stream-control response sends a per-camera `EventAIPortStatus` with the observed streaming state. Smart-detection and audio-event readiness are false by default. A separate expiring, one-camera probe temporarily sends `EventFeatureFlagsUpdated` with one selected object capability and reports smart readiness. The candidate classifies a narrow `ChangeSmartDetectSettings` subset for the exact camera: full-frame person, vehicle and animal detection, or primary-lens polygons for those classes, with bounded event timing. Protect 7.3.60 sent `enableSmartDetect: []` alongside a Person detection zone for a paired legacy G3 camera. The parser derives supported classes only from validated primary-lens zones in that case; mixed Person, Vehicle and Animal zones are covered by synthetic tests, not a native Protect trial. An empty list with no usable zone remains disabled. Validated exclusion polygons conservatively suppress any supported object box they overlap, including when full-frame detection is otherwise enabled. This exclusion behavior has synthetic tests but no live Protect validation. Secondary-lens zones, lines, tamper, access triggers and PTZ remain unsupported. The settings-only probe counts matching requests and still replies unsupported; it never retains the nested policy.

An additional private `diagnostic_event_until` field can enable one single-class event trial. It must equal both the stream and smart-probe deadline, which can be at most ten minutes ahead. A model-backed trial also requires a local detector policy with 2-120 frame attempts, or up to 600 attempts when an already paired stream supplies the frames. The default class is person; `diagnostic_smart_type` may explicitly select vehicle or animal for a separate bounded model test. Only a matching single-class setting for the selected camera receives an acknowledgement. If reverification is enabled with a valid integer probability range, the candidate drops observations at or below its upper bound before temporal tracking; it does not perform the requested second-stage verification. For a supported primary-lens zone, the candidate requires at least 90% of the object box area inside a simple polygon, then includes that zone ID in the native event candidate. Configured zones with unsupported geometry or access triggers are rejected. This provisional overlap rule is informed by a [Ubiquiti team explanation for vehicle LPR unlock zones](https://community.ui.com/questions/AI-Pro-License-Plate-Unlock/fccc7c12-3a6d-4efe-9678-5bd8db1b88d6), not proof of the Person threshold on AI Port. It does not calibrate camera zone sensitivity. A later disabled or unsupported setting revokes the policy. After two overlapping qualifying model observations, the in-memory tracker may send one `EventSmartDetect` enter, at most one moving update per second while the same track remains inside the same zone, and a leave when the track disappears. The moving update carries a refreshed descriptor without another zone-enter status. These moving updates pass synthetic per-camera tests but have not been accepted by Protect 7.3.60. The payload includes a wall-clock timestamp and normalized box. When a JPEG frame is available, its leave edge also references one cropped and one full-frame in-memory smart-detection snapshot. It makes no face, plate, speech or camera-credential claim. Protect 7.3.60 indexed live Person results on the original G3 camera in a later three-stream trial, with playback association. Repeatability, vehicle and animal live acceptance, full reverification and alarms remain `needs_evidence`. The default passive profile emits no smart events.

For a protocol-only original-timeline test when no person enters the camera view, `diagnostic_native_event_probe` can replace `diagnostic_detector`. It is off unless a private config names one exact stream camera, a new 32-character lowercase hex nonce, and a normalized `[x1, y1, x2, y2]` box. `diagnostic_hello_until`, `diagnostic_smart_probe_until`, and `diagnostic_event_until` must have the same expiry, at most ten minutes ahead. The controller must pair that camera, start its stream, and send an enabled Person policy. The configured box must pass that policy's zone and confidence checks. The candidate claims the nonce in a mode-600 state marker **before** sending one synthetic Person enter and a leave two seconds later. A restart with the same private state cannot replay it. Health exposes only claim and error counters. This is a deliberate false test event, not a model result; it can appear in the camera timeline and trigger configured alarms. Use it only during an approved test after checking alarm actions, then remove the diagnostic fields. Keep an already paired camera paired unless the operator asks to remove it. Native persistence remains `needs_evidence`.
...
Replace every placeholder with current private values; the shown timestamps are examples and will be rejected by the 24-hour check. The candidate checks both JPEG hashes again at use, runs the pinned local model, requires exactly one overlapping person in each frame, and applies Protect's current confidence and zone policy to both observations. Only then does it durably claim the nonce and send native enter, moving and leave edges. Enter uses the first frame's observed position and capture time; moving uses the second frame's position and capture time. The two descriptors share the track ID confirmed by the two-hit tracker. The candidate delivers enter and moving at the recorded frame spacing, then leave two seconds later. The native payload contains a class, confidence, normalized box, and, when the JPEG is valid, a snapshot reference. The candidate holds one crop and one full-frame JPEG in memory and sends each only if the pinned controller issues an exact `GetRequest` for its filename at its one-use camera-upload endpoint. This does **not** cryptographically prove the frames or capture times came from Protect, and an old event may be rejected or create a duplicate event or notification. Test with an existing recording only when that duplicate is acceptable. The feature is off by default. The earlier Flur G3 trials sent enter/leave pairs and showed Smart cards at the old recording time, including after a full page reload while paired. A fourth bounded trial sent the moving-edge variant; Protect still returned zero Flur Person search results while paired. Whether the moving update changed the saved event or its class is `needs_evidence` because that trial's subscription watcher did not finish. Alarm behavior, long-term persistence, live operation and other cameras also remain `needs_evidence`. After each trial, the camera's AI Event recording was disabled again, Flur was unpaired, and the candidate restarted with passive settings.

A 24 September live legacy-G3 trial produced a model-backed Person event in Protect's All Detections, with a `smartDetectType:person` device-service label, but no Person search result. The device-service log reported no snapshots for that event, while a native camera event inserted a smart-detect object after its snapshot upload. This is evidence of a missing snapshot path, not proof that snapshots alone fix Person indexing. The candidate now includes object and full-frame snapshot references on the leave edge and handles the controller's one-use JPEG upload request with a pinned TLS connection. The Protect 7.3.60 support log showed one-use camera-upload tokens as 36-character UUIDs. The earlier candidate expected 32 characters and rejected these URLs. Its strict UUID validator accepted all 49 saved smart-detection crop and full-frame requests in an offline check. The replay below verifies the upload transport; a nonempty Person search after reload remains `needs_evidence`.

A later one-use replay stopped at the Person zone gate. Offline comparison of the approved frames with the saved nearly full-frame zone found that the person's bounding box crossed its lower edge but still overlapped the zone by more than 90%. The provisional overlap rule above admits both frames in an offline rerun.

The bounded replay with that rule subsequently sent one Person enter, moving update and leave while the Flur G3 stream remained paired. Protect requested the cropped and full-frame snapshots; both uploads returned HTTP 200. A separate two-minute, read-only Protect integration-API subscription for exactly Flur saw one smart-video add and three updates, each with an explicit Person class, and zero invalid messages. The subscription is strong evidence that Protect announced a Person event during the replay, but it does not prove long-term timeline persistence, a nonempty Person search after reload or sustained live detection. The saved container configuration returns to passive paired mode on restart.

The first trial sent enter and leave back-to-back even though their `clockWall` values were two seconds apart. The second trial waited two real seconds between those edges and checked that the same stream, policy and expiring permit remained active. Flur was paired and **Record AI Events** was enabled, but Protect's Flur **Person** filter still showed **0 Results while paired**. Pacing alone did not establish class indexing. After the trial, Flur's AI Event recording was disabled, the camera was unpaired, and the candidate restarted with no diagnostic policy. The original cause remains `needs_evidence`.

A third bounded trial used a fresh one-use permit on the same private recording with the updated read-only watcher. The candidate again reported one qualified Person enter/leave pair without an error. Protect showed another Smart card at the historical time, but its Flur Person filter still returned zero results while paired. The subscription observed one smart-video add and one update, **both with an explicit empty `smartDetectTypes` list**; neither message omitted that field. This narrows the failure to Protect leaving the class unset, without proving whether descriptor validation, event handling or another gate caused it. Flur's AI Event recording was turned off, Flur was unpaired, and the candidate restarted from its five-field passive config with ingress disabled and the adopted control connection active. The controller-side cause remains `needs_evidence`.
