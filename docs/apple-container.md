...
These results cover management and control. No native camera-frame description, persistent description or search result has been verified. Face/plate recognition, audio and full legacy-camera enhancement are not implemented. Search and PostgreSQL remained disabled. NAS deployment is untested. The static protocol research uses Protect 7.2.105 and AI Key firmware 2.2.8; it is distinct from these live 7.3.56 checks.
...
Stop the test with `container stop local-aikey-mac`. Retain the state directory for recovery. Adoption and reconnect passed in the tested setup. One actual native description, persistence, search retrieval and longer recovery tests remain separate acceptance checks.
