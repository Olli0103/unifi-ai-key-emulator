...
The JSON names the number of instances and Apple container TCP publish bindings. Unassigned instances have no binding, so a deployment controller must refuse to start them until it has distinct, conflict-checked IPs. A container cannot add host-published ports to itself after startup; a host deployment controller must refresh inventory and create or reconcile the required instances. The current code is an automatic camera-detection and capacity preflight. It is not yet an AI Port service or an automatic container launcher.

The next native test is an isolated HTTPS 443 management service, with bounded setup authentication, followed by adopted WebSocket reconnect. Camera pairing waits for a verified device-side message contract and a recorded rollback for the selected camera.
