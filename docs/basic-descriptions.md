...
For a camera whose native recognition results must be preserved, an individual `recognizeKeyFrames` test scope may set `"callback_profile": "description_only"`. This sends only `cameraId`, `eventId`, `status`, and `description` through the source-evidenced multipart RAM description branch. It never submits `keyMomentsTags: []`. The default `full` profile and continuous mode are unchanged. The description-only receiver updates an existing RAM row rather than inserting one, so its persistence on a G6 camera is `needs_evidence`; HTTP 200 alone does not settle that. A fresh event, its existing native recognition results, and the caption must be checked before broader admission. Each profile still consumes a separate one-use permit before media or inference.
