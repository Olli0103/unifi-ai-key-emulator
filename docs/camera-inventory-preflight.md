...
The command writes `camera-inventory-preflight.json` and a static `camera-inventory-preflight.html` beside the API key with mode 0600 and prints counts only. Open the HTML file locally to review names, models, Protect-reported smart types and reasons. It has no JavaScript or external resources. `smart_event_candidate` means Protect reports smart detection types; it does not prove the camera itself provides them or that the AI Key emulator will receive or persist an event. In the live 7.3.60 inventory, G3–G5 cameras configured for AI Port also reported smart types. `legacy_ingress_needed` means the camera reports no smart detection types, so an AI Port profile or another native-verified ingress path is needed before legacy enhancement. `offline` means no processing can be inferred while the camera is disconnected. All report entries have `processing_enabled: false` at the report level. The private JSON includes validated camera MACs for matching an AI Port allowlist; the static HTML does not display them.
