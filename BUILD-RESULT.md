...
The result is a runnable experimental implementation with source, tests, a Python package, Docker files and NAS instructions. One native on-demand description is now verified on Protect 7.3.60. Persistence and native search remain unverified. The automated local tests use fixtures; the separate live checks below contacted the authorized controller and provider.
